...
annotations_path =      "/home/elisa/Data/Ho3D/segmentation/train"
annotations_json_file = "annotations.json"
imageset_file =         "/home/elisa/Data/Ho3D/ImageSets/imageset_test.txt"
...
ho3d_path = os.path.abspath('/home/elisa/Data/Ho3D')
ho3d_path_Json = os.path.abspath(os.path.join(os.getcwd(), 'HO3D_V2_Json_format'))
...
def create_images_array(image_to_idx):
    ...
    for img_name, img_id in image_to_idx.items():
        image_dict = {
            "id"        : img_id,
            "license"   : 1,
...
def create_annotations_array(array, dir_name, img_name, objects_data, image_to_idx, cls_to_idx, annotations_id):

    img_id = image_to_idx[dir_name + '/' + img_name]
    ...
    for obj in objects_data:
        if not obj['label'] == 'dummy':
            ...
            h = ymax - ymin
            area = w*h
            #print(str(xmin) + ' ' + str(xmax) + ' ' + str(ymin) + ' ' + str(ymax) + ' ' + str(h) + ' ' + str(w) + ' ' + str(area) )

            object_dict = {
                "id"          : annotations_id,
                "image_id"    : img_id,
                "image_name"  : dir_name + '/' + img_name,
...
#        os.mkdir(annotations_dir)
    ...
    for segm_file in segm_files:
        ...
        if not obj_dir + '/' + img_name in image_to_idx_dict:
            continue
        ...
        annotations_array, annotations_id = create_annotations_array(annotations_array, obj_dir, img_name, objects, image_to_idx_dict, dict_cls_to_id, annotations_id)
